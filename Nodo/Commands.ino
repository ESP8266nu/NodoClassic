...
boolean ExecuteCommand(struct NodoEventStruct *EventToExecute)
  ...
  switch(EventToExecute->Command)
    ...
    case CMD_FLAG_SYNC:
      ...
      if((EventToExecute->SourceUnit==Settings.Unit) || (EventToExecute->SourceUnit==0)) // Nodo Classic, change to make nodotool test easier
        {
        TempEvent.SourceUnit = Settings.Unit; // Nodo Classic
        TempEvent.Type=NODO_TYPE_COMMAND;
